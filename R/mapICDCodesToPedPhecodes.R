...
mapICDCodesToPedPhecodes <-
  function(input) {
    ...
    withCallingHandlers(output <- merge(input,PediatricPheCodes::vocabulary_map,by=c("vocabulary_id","code")),
    ...
    #Perform the rollup
    withCallingHandlers(output <- merge(output,PediatricPheCodes::rollup_map,by="code"),
                        warning = function(w) { if (grepl("coercing into character vector", w$message)) {invokeRestart("muffleWarning")}})
